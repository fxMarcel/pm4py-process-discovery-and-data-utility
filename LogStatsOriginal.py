...
def evaluate_for_threshold(threshold, original_log, anonymized_log):
    ...
    # F1-Score berechnen
    f1_score = (2 * precision * fitness / (precision + fitness)) if (precision + fitness) > 0 else 0.0
...
def main():
    ...
    # Log-Pfade
    original_log_path = os.path.join(metrics_dir, "20250414_spaghetti_event_log_v2_modified.xes")
    anonymized_log_path = os.path.join(metrics_dir, "20250414_spaghetti_event_log_v2_modified.xes")
    ...
    # JSON speichern
    json_path = os.path.join(metrics_dir, "benchmarking_results_spaghetti.json")
    with open(json_path, "w") as f:
        json.dump(all_results, f, indent=4)

    # Plot speichern
    plot_path = os.path.join(metrics_dir, "benchmarking_plot_spaghetti.png")
